...
export async function getNavigationLinks(): Promise<NavigationLink[]> {
	...
	try {
		...
		const blogPage = pages.find((page) => page.slug === "blog");

		const links: NavigationLink[] = [{ title: "Home", href: "/" }];
		...
		console.error("Failed to load navigation links from WordPress", error);
		return fallback;
	}
}
